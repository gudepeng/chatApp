	...
	<div @longpress="long">
		...
		:class="!isself ? 'justify-start' : 'justify-end'">
			...
			<template v-if="!isself">
				<fss-avater size="70" :src="item.from_avatar" @click="openUser"></fss-avater>
			...
			<view class="flex flex-column">
				...
				<div class="p-2 rounded" :class="labelClass" style="max-width:500rpx;" :style="labelStyle">
					...
					<!-- 表情包 | 图片-->
					<fss-image @longpress="stop" v-else-if="item.type === 'emoticon' || item.type === 'image'" :src="item.data" @click="preview(item.data)" imageClass="rounded" :maxWidth="500" :maxHeight="350"></fss-image>
					...
					@tap="openVideo" @longpress="stop">
						<fss-image :src="item.options.poster" imageClass="rounded" :maxWidth="300" :maxHeight="350" @load="loadPoster"></fss-image>
			...
			<template v-if="isself">
				...
				<fss-avater size="70" :src="item.from_avatar"
				@click="openUser"></fss-avater>
			</template>
		</view>
...
<script>
	...
	import fssImage from './ui-image.vue';
